...
自製的 **ICCAD Contest 2026 Problem A: LLM-Assisted Netlist Exploration and Transformation** benchmark 公開題庫。
...
Runner 會繼承呼叫者的 **環境變數**,所以系統啟動時要讀的東西（API key、`LD_LIBRARY_PATH`、shell wrapper 裡的 config 路徑）都直接從 shell 來。Runner 只額外注入 `LD_LIBRARY_PATH=/lib64/:$LD_LIBRARY_PATH`,確保 NTHU CAD 工作站上 PyYAML 能 import。
...
- 2026-05-07:Initial scaffold + 6 個 sample testcase（contest §4 例子 + 小型 demo design）。
