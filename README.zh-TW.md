...
# 2. 執行 runner（NTHU CAD 工作站請用 python3.9）
python3.9 runner/run_bench.py --source official --cases test01      # 單一官方 testcase
python3.9 runner/run_bench.py --source community --cases demo01     # 單一社群 testcase
python3.9 runner/run_bench.py --source all                          # 全部
...
你的 `python3` 版本低於 3.7。在 NTHU CAD 工作站上請改用 `python3.9`。

**`spawn failed` 或 `No such file or directory`（找不到系統執行檔）**
`BENCH_SYSTEM_CMD` 必須使用**絕對路徑**。Runner 在暫存目錄執行，相對路徑無法解析。

**`[fatal] config not found: …`**
確認傳給系統的 config 檔名是否正確。常見錯誤是寫了 `config.yaml`，但實際檔名是 `llm_config.yaml`。

**CI 報錯：「submission folder does not match PR author」**
答案子資料夾名稱必須與你的 GitHub 帳號完全一致（大小寫敏感）。請重新命名資料夾。

**CI 報錯：「expected N response block(s)」**
Log 裡的 `#RESPONSE`/`#END` block 數量必須等於 `requests.txt` 中非註解行的數量。請對完整 testcase 重跑系統。
